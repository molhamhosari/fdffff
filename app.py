...
def chat():
    try:
        ...
        try:
            ...
            app.logger.error(f"Language detection error: {str(e)}", exc_info=True)
            return jsonify({"response": "Could not detect the language. Please provide a valid message."}), 400
